...
class AnimationRunner(bpy.types.Operator):
    """Operator which runs its self from a timer"""
    bl_idname = "wm.opencv_operator"
    bl_label = "OpenCV Animation Operator"
     
    # p = our pre-treined model directory
    #p = "D:\\path\\to\\shape_predictor_68_face_landmarks.dat" # Windows
    #p = "/path/to/shape_predictor_68_face_landmarks.dat" # macOS
    p = "C:\\Users\\ijh19\\OneDrive\\Desktop\\Projects\\adv_graphics_tutorial\\shape_predictor_68_face_landmarks.dat" # linux
    ...
    def modal(self, context, event):
        ...
        if event.type == 'TIMER':
            ...
            for (i, rect) in enumerate(rects):
                ...
                bones["FK-HNG-Head"].rotation_euler[0] = self.smooth_value("h_x", 3, (self.rotation_vector[0] - self.first_angle[0])) / 1   # Up/Down
                bones["FK-HNG-Head"].rotation_euler[2] = self.smooth_value("h_y", 3, -(self.rotation_vector[1] - self.first_angle[1])) / 1.5  # Rotate
                bones["FK-HNG-Head"].rotation_euler[1] = self.smooth_value("h_z", 3, (self.rotation_vector[2] - self.first_angle[2])) / 1.3   # Left/Right
                
                bones["Jaw"].location[2] = self.smooth_value("m_h", 2, self.get_range("mouth_height", numpy.linalg.norm(shape[62] - shape[66])) * 0.06)
                bones["CON-Eyebrow1.L"].location[1] = self.smooth_value("b_l", 3, (self.get_range("brow_left", numpy.linalg.norm(shape[19] - shape[27])) -0.5) * 0.04)
                bones["CON-Eyebrow1.R"].location[1] = self.smooth_value("b_r", 3, (self.get_range("brow_right", numpy.linalg.norm(shape[24] - shape[27])) -0.5) * 0.04)
                
                bones["FK-HNG-Head"].keyframe_insert(data_path="rotation_euler", index=-1)
                bones["Jaw"].keyframe_insert(data_path="location", index=-1)
                bones["CON-Eyebrow1.L"].keyframe_insert(data_path="location", index=2)
                bones["CON-Eyebrow1.R"].keyframe_insert(data_path="location", index=2)
